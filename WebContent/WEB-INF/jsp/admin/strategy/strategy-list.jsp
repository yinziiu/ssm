...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>攻略管理 <span class="c-gray en">&gt;</span> 攻略列表
...
<div class="page-container">
	<div class="text-c"> 日期范围：
		...
		<input type="text" name="" id="" placeholder=" 攻略名称" style="width:250px" class="input-text">
		<button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i>搜攻略</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a class="btn btn-primary radius" onclick="add('添加攻略','${path}/addStrategy')" href="javascript:;">
			<i class="Hui-iconfont">&#xe600;</i> 添加攻略</a>
		</span>
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<c:forEach items="${strategiesList }" var="list">
				<tr class="text-c va-m">
					...
					<td class="text-l">${list.stname }</td>
					<td class="text-l">${list.stcontent }</td>
					<td class="text-l">${list.sttime }</td>
					<td class="text-l">
						<c:choose>
							<c:when test="${list.stphoto==null || list.stphoto =='' }">					
								<img width="100px" height="100px" src="${path }/images/icon/view.jpg">
							</c:when>
							<c:otherwise>
								<img width="100px" height="100px" src="${list.stphoto }" />
							</c:otherwise>
						</c:choose>
					</td>
					<td class="text-l">${list.comefrom }</td>
					<td class="text-l">${list.streadnum }</td>
					<td class="f-14 user-manage">
       					<a style="color: orange;" title="编辑" onclick="edit('编辑景点','${path}/updateStrategy?id=${list.id }')"  class="ml-5">编辑</a> 
	    				<a style="color: orange;" title="删除" onclick="del(this,'${path}/deleteStrategy?id=${list.id  }')"  class="ml-5">删除</a>
       				</td>
       			</tr>
			</c:forEach>
			</tbody>
