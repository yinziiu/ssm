...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>点评管理 <span class="c-gray en">&gt;</span> 酒店点评列表
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<c:forEach items="${hotelcommentList }" var="list">
				<tr class="text-c va-m">
					...
					<td class="text-l">${list.hcomment }</td>
					<td class="text-l">${list.hcommenttime }</td>
					<td class="text-l">
						<c:choose>
							<c:when test="${list.hphoto==null || list.hphoto =='' }">					
								<img width="100px" height="100px" src="${path }/images/icon/hotel.jpg">
							</c:when>
							<c:otherwise>
								<img width="100px" height="100px" src="${path }${list.hphoto }" />
							</c:otherwise>
						</c:choose>
					</td>
					<td class="f-14 user-manage">
       					<a style="color: orange;" title="删除" onclick="hcomment_del(this,'${path}/deleteHComment?id=${list.id }')"  class="ml-5">删除</a>
       				</td>
       			</tr>
			</c:forEach>
			</tbody>
