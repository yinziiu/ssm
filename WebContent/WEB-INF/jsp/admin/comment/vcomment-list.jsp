...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>点评管理 <span class="c-gray en">&gt;</span> 景点点评列表
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<c:forEach items="${viewcommentList }" var="list">
				<tr class="text-c va-m">
					...
					<td class="text-l">${list.vcomment }</td>
					<td class="text-l">${list.vcommenttime }</td>
					<td class="text-l">
						<c:choose>
							...
							<c:otherwise>
								<img width="100px" height="100px" src="${path }${list.vphoto }" />
							</c:otherwise>
						</c:choose>
					</td>
					<td class="f-14 user-manage">
       					<a style="color: orange;" title="删除" onclick="vcomment_del(this,'${path}/deleteVComment?id=${list.id }')" class="ml-5">删除</a>
       				</td>
       			</tr>
			</c:forEach>
			</tbody>
