...
<html>
	...
	<body>
		<div class="Huiform">
			<form class="form form-horizontal" id="user-add" method="post" enctype="multipart/form-data">
				...
				<div class="row cl" >
					<label class="form-label col-xs-4 col-sm-3">角色：</label>
					<div class="formControls col-xs-4 col-sm-5">
						<input type="text" id="name" name="name" value="用户" class="input-text" readonly="readonly">
					</div>
				</div>
				<div class="row cl">
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-4 col-sm-5">
						<input type="text" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'birthday\')||\'%y-%M-%d\'}' })" id="birthday" name="birthday" class="input-text Wdate">
					</div>
				</div>
				<div class="row cl">
		...
		<script type="text/javascript">
			...
			function save(){
	       		var formData = new FormData($("#user-add")[0]);//此处id为form表单的id
		        $.ajax({  
		            url: "${path}/saveUser" ,  
		            type: 'POST',  
		            data: formData,  
