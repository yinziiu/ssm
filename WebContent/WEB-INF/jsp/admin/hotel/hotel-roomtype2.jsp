...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>酒店管理 <span class="c-gray en">&gt;</span> 房型管理
...
<div class="page-container">
	<div class="text-c"> 日期范围：
		...
		<input type="text" name="" id="" placeholder=" 酒店房型" style="width:250px" class="input-text">
		<button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i>搜酒店房型</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a class="btn btn-primary radius" onclick="add('添加房型','${path}/HotelRoomType_add?hid=${id }')" href="javascript:;">
			<i class="Hui-iconfont">&#xe600;</i> 添加酒店房型</a>
		</span>
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<c:forEach items="${HotelRoomType }" var="list">
				<tr class="text-c va-m">
					<td><input name="" type="checkbox" value=""></td>
					<td class="text-l">${list.id }</td>
					<td class="text-l">${list.hname }</td>
					<td class="text-l">${list.htype }</td>
					<td class="text-l">${list.hnum }</td>
					<td class="text-l">${list.hprice }</td>
					<td class="text-l">${list.roomtotal }</td>
					<td class="text-l">
						<u style="cursor:pointer" class="text-primary" onclick="picture_show('${path}/getHotelRoomPhoto?id=${list.id }')">
							<c:choose>
								<c:when test="${list.roomphoto==null || list.roomphoto =='' }">					
									<img width="80px" height="80px" src="${path }/images/icon/hotel.jpg">
								</c:when>
								<c:otherwise>
									<img width="80px" height="80px" src="${path }${list.roomphoto }" />
								</c:otherwise>
							</c:choose>
						</u>
					</td>
					<td class="f-14 user-manage">
       					<a style="color: orange;" title="编辑" onclick="edit('编辑酒店房型','${path}/updateHotelRoomType?id=${list.id }')"  class="ml-5">编辑</a> 
	    				<a style="color: orange;" title="删除" onclick="del(this,'${path}/deleteHotelRoomType?id=${list.id  }')"  class="ml-5">删除</a>
       				</td>
       			</tr>
			</c:forEach>
			</tbody>
