...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>酒店管理 <span class="c-gray en">&gt;</span> 酒店类型列表
...
<div class="page-container">
	<div class="text-c"> 日期范围：
		...
		<input type="text" name="" id="" placeholder=" 酒店类型" style="width:250px" class="input-text">
		<button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i>搜酒店类型</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a class="btn btn-primary radius" onclick="hotel_type_add('添加酒店类型','${path}/HotelType_add')" href="javascript:;">
			<i class="Hui-iconfont">&#xe600;</i> 添加酒店类型</a>
		</span>
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<c:forEach items="${HotelTypeList }" var="list">
				<tr class="text-c va-m">
					<td><input name="" type="checkbox" value=""></td>
					<td class="text-l">${list.id }</td>
					<td class="text-l">${list.hoteltype }</td>
					<td class="f-14 user-manage">
       					<a style="color: orange;" title="编辑" onclick="hotel_type_edit('编辑酒店类型','${path}/updateHotelType','${list.id }')"  class="ml-5">编辑</a> 
       					<a style="color: orange;" title="删除" onclick="hotel_type_del(this,'${path}/deleteHotelType?id=${list.id }')"  class="ml-5">删除</a>
       				</td>
       			</tr>
			</c:forEach>
			</tbody>
...
/*酒店类型-添加*/
function hotel_type_add(title,url){
...
/*酒店类型-编辑*/
function hotel_type_edit(title,url,id){
	var index = layer.open({
		type: 2,
		title: title,
		area: ['90%', '50%'],
...
/*酒店类型-删除*/
function hotel_type_del(obj,url){
