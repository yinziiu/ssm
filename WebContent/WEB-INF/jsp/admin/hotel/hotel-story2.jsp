...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>酒店管理 <span class="c-gray en">&gt;</span> 酒店攻略列表
...
<div class="page-container">
	<div class="text-c"> 日期范围：
		...
		<input type="text" name="" id="" placeholder=" 酒店攻略" style="width:250px" class="input-text">
		<button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i>搜酒店攻略</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<c:if test="${totalList == 0 }">
			<span class="l">
				<a class="btn btn-primary radius" onclick="hotelstory_add('添加酒店攻略','${path}/HotelStory_add?hid=${id }')" href="javascript:;">
				<i class="Hui-iconfont">&#xe600;</i> 添加酒店攻略</a>
			</span>
		</c:if>
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<c:forEach items="${HotelStoryList }" var="list">
				<tr class="text-c va-m">
					<td><input name="" type="checkbox" value=""></td>
					<td class="text-l">${list.id }</td>
					<td class="tect-l">${list.hname }</td>
					<td class="text-l">${list.gushi }</td>
					<td class="f-14 user-manage">
       					<a style="color: orange;" title="编辑" onclick="edit('编辑景点','${path}/updateHotelStory?id=${list.id }')"  class="ml-5">编辑</a> 
	    				<a style="color: orange;" title="删除" onclick="del(this,'${path}/deleteHotelStory?id=${list.id  }')"  class="ml-5">删除</a>
       				</td>
       			</tr>
			</c:forEach>
			</tbody>
...
/*酒店攻略-添加*/
function hotelstory_add(title,url){
