...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>景点管理 <span class="c-gray en">&gt;</span> 景点类型管理
...
<div class="page-container">
	<div class="text-c"> 日期范围：
		...
		<input type="text" name="" id="" placeholder="景点类型" style="width:250px" class="input-text">
		<button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i>搜景点类型</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a class="btn btn-primary radius" onclick="view_type_add('添加景点类型','${path}/ViewType_add')" href="javascript:;">
			<i class="Hui-iconfont">&#xe600;</i> 添加景点类型</a>
		</span>
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<c:forEach items="${viewtypeList }" var="list">
				<tr class="text-c va-m">
					<td><input name="" type="checkbox" value=""></td>
					<td class="text-l">${list.id }</td>
					<td class="text-l">${list.viewtype }</td>
					<td class="f-14 user-manage">
       					<a style="color: orange;" title="编辑" onclick="view_type_edit('编辑景点类型','${path}/updateViewType?id=${list.id }')"  class="ml-5">编辑</a> 
       					<a style="color: orange;" title="删除" onclick="view_type_del(this,'${path}/deleteViewType?id=${list.id }')"  class="ml-5">删除</a>
       				</td>
       			</tr>
			</c:forEach>
			</tbody>
...
/*景点类型-添加*/
function view_type_add(title,url){
...
/*景点类型-编辑*/
function view_type_edit(title,url){
	var index = layer.open({
		type: 2,
		title: title,
		area: ['90%', '50%'],
...
/*景点类型-删除*/
function view_type_del(obj,url){
