...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>景点管理 <span class="c-gray en">&gt;</span> 景点门票列表
...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a class="btn btn-primary radius" onclick="add('添加景点门票','${path}/add_ticket?vid=${id }')" href="javascript:;">
			<i class="Hui-iconfont">&#xe600;</i> 添加门票类型</a>
		</span>
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<c:forEach items="${viewTickets }" var="list">
				<tr class="text-c va-m">
					...
					<td>${list.id }</td>
					<td>${list.vname }</td>
					<td class="text-l">${list.viewphoto }</td>
					<td class="text-l">${list.tname }</td>
					<td class="text-l">${list.ttype }</td>
					<td class="text-l">${list.tprice }</td>
					<td class="text-l">${list.sales }</td>
					<td class="f-14 user-manage">
       					<a style="color: orange;" title="编辑" onclick="edit('编辑景点门票','${path}/update_ticket?id=${list.id }')"  class="ml-5">编辑</a> 
	    				<a style="color: orange;" title="删除" onclick="del(this,'${path}/deleteViewTicket?id=${list.id  }')"  class="ml-5">删除</a>
       				</td>
       			</tr>
			</c:forEach>
			</tbody>
