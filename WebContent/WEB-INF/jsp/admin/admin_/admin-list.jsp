...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a class="btn btn-primary radius" onclick="add('添加管理员','${path}/AddAdmin')" href="javascript:;">
			...
			<c:forEach items="${adminList }" var="user">
		      <tr class="text-c">
	        	...
	        	<td class="f-14 user-manage">
    				<a style="color: orange;" title="编辑" onclick="edit('编辑用户','${path}/UpdateAdmin?id=${user.id }')"  class="ml-5">编辑</a> 
    				<c:if test="${adminuser.name eq '超级管理员' }">
    					<a style="color: orange;" title="删除" onclick="del(this,'${path}/deleteAdmin?id=${user.id }')"  class="ml-5">删除</a>
    				</c:if>
    			</td>
	      	</tr>
	   	</c:forEach>
