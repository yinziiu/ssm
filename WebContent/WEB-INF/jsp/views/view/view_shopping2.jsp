		...
		<div class="container">
			<%-- <form id="formFilter" action="${path }/web/FindTicket" method="post">
				<div class="filter-panel">
					<div class="filter-hd clearfix" id="saleType">
						...
						<input style="margin: 0;padding: 0;border: 1px solid transparent;outline: none;width: 300px;height: 40px;margin-left: 30px;padding-left: 10px; " type="text" name="tname" id="tname" value="请输入门票名称进行查询" />
			...
			<div class="sales-content">
				...
				<c:if test="${dbuser!=null }">
					<c:if test="${viewTickets==null }">
						<p>您搜索的门票暂时没有，请换条件搜索......</p>
					</c:if>
						...
						<a class="item clearfix" href="${path }/web/view_order?id=${list.id}&&vid=${list.vid}" target="_blank">
							<div class="image">
								<c:choose>
									...
									<c:otherwise>
										<img class="lazy" width="220" height="150" src="${list.viewphoto }" />
									</c:otherwise>
								</c:choose>
						...
						<li>
							<a href="${path }/web/view_order?id=${list.id}&&vid=${list.vid }" target="_blank">
								<span class="image">
									<c:choose>
										<c:when test="${list.viewphoto==null || list.viewphoto =='' }">
											<img class="lazy" width="100" height="70" src="${path}/images/icon/view.jpg">
										</c:when>
										<c:otherwise>
											<img class="lazy" width="100" height="70" src="${list.viewphoto }" />
										</c:otherwise>
									</c:choose>
								</span>
