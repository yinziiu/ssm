						...
						<c:forEach items="${foodComments }" var="list">
							<div class="poi-item  have _j_poiitem _j_poiitem_5429128 _j_commentitem_190312744" >
						        	...
						        	<a href="${path }/web/food_detail?fid=${list.id}" target="_blank">
						        		<c:choose>
											<c:when test="${list.fphoto==null || list.fphoto =='' }">
												<img src="${path}/images/icon/food.jpg" height="120" width="120" alt="${list.fname }">
											</c:when>
											<c:otherwise>
												<img src="${list.fphoto }" height="120" width="120" alt="${list.fname  }" />
											</c:otherwise>
										</c:choose>
						        	</a>
						         </span>
								<div class="poi-detail">
									...
									<div class="poi-photo" >
										<a href="${path }/web/food_detail?fid=${list.id}">
											<c:choose>
												<c:when test="${list.fcphoto==null || list.fcphoto =='' }">
													<img class="_j_poi_photo" src="${path}/images/icon/food.jpg" width="100px" height="100px">
												</c:when>
												<c:otherwise>
													<img class="_j_poi_photo" src="${list.fcphoto }" width="100px" height="100px" />
												</c:otherwise>
											</c:choose>
										</a>
									</div>
								...
								<div class="poi-btns">
									<span class="time">${list.fcommenttime }</span> </div>
								</div>
							</c:forEach>
						</div>
					</div>
