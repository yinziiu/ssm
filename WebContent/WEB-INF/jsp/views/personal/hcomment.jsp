						...
						<c:forEach items="${hotelComments }" var="list">
							<div class="poi-item  have _j_poiitem _j_poiitem_5429128 _j_commentitem_190312744" >
						        	...
						        	<a href="${path }/web/hotel_detail?hid=${list.id}" target="_blank">
						        		<c:choose>
											<c:when test="${list.hphoto==null || list.hphoto =='' }">
												<img src="${path}/images/icon/hotel.jpg" height="120" width="120" alt="${list.hname }">
											</c:when>
											<c:otherwise>
												<img src="${path}${list.hphoto }" height="120" width="120" alt="${list.hname  }" />
											</c:otherwise>
										</c:choose>
						        	</a>
						         </span>
								<div class="poi-detail">
									<h3 class="title">
										<a href="${path }/web/hotel_detail?hid=${list.id}" target="_blank">${list.hname }</a>
									</h3>
									<div class="poi-rev _j_comment">
										${list.hcomment }
									</div>
									<div class="poi-photo" >
										<a href="${path }/web/hotel_detail?hid=${list.id}">
											<c:choose>
												<c:when test="${list.hcphoto==null || list.hcphoto =='' }">
													<img class="_j_poi_photo" src="${path}/images/icon/hotel.jpg" width="100px" height="100px">
												</c:when>
												<c:otherwise>
													<img class="_j_poi_photo" src="${path}${list.hcphoto }" width="100px" height="100px" />
												</c:otherwise>
											</c:choose>
										</a>
									</div>
								...
								<div class="poi-btns">
									<span class="time">${list.hcommenttime }</span> </div>
								</div>
							</c:forEach>
						</div>
					</div>
