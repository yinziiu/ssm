				...
				<div class="home-extra">
					...
					<div class="order-table">
						...
						<c:forEach items="${hotelOrders }" var="list">
							<table data-id="19889738" data-type="hotel_youyu" class="order-item">
								<caption>
									<span class="num">订单号：
                                        <a href="${path }/web/myhotelorder_detail?id=${list.id}" target="_blank">
					                       ${list.hordernum }
					                    </a>
                                    </span>
									<span class="time">下单时间：${list.booktime }</span>
									<span class="telphone">联系电话：${list.phone }</span>
								</caption>
								<tbody>
									<tr>
										<td class="td-info">
											<a class="pro-img" href="${path }/web/hotel_detail?id=${list.id}" target="_blank">
												<c:choose>
													<c:when test="${list.hphoto==null || list.hphoto =='' }">
														<img src="${path}/images/icon/view.jpg" alt="${list.hname }">
													</c:when>
													<c:otherwise>
											...
											<div class="pro-detail">
												<p class="title" title="${list.hname }">
													<a href="${path }/web/hotel_detail?id=${list.id}" target="_blank">${list.hname }</a>
												</p>
												<p class="sub" title="门票 × 3">房间 × ${list.roomnum }</p>
											</div>
										</td>
										<td class="td-type">酒店</td>
										<td class="td-date">
											<p>入住: ${list.checkintime }</p>
											<p>离店: ${list.checkouttime }</p>
										</td>
										<td class="td-cost">
											<strong>￥${list.hprice }</strong> 
										</td>
										<c:if test="${list.status eq '待支付' }">
											...
											<td class="td-operate">
												...
												<a class="btn" href="${path }/web/payHotelOrder?id=${list.id}" target="_blank">
													去支付
												</a>
												<form action="${path }/web/quxiaoStatus" method="post">
										...
										<c:if test="${list.status eq '已关闭' }">
											...
											<td class="td-operate">
												<a  href="${path }/web/deleteHotelOrder?id=${list.id}">
													删除订单
												</a>
												...
												<a href="${path }/web/hotel_detail?id=${list.id}" target="_blank">
													重新预订
												</a>
											</td>
										</c:if>
